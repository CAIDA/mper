		       ...
		       size_t *msg_length_out)
{
  ...
  words[0].cw_name = keyword_code_names[KC_REQNUM];  /* for completeness */
  ...
  /* somewhat convoluted way of getting the type since type isn't exported */
			...
			size_t *msg_length_out)
{
  ...
  for (i = 2; i < length; i++) {
    ...
    case KT_STR:
    case KT_BLOB:
      if (words[i].cw_len > MPER_MSG_MAX_RAW_VALUE_SIZE) {
	sprintf(message_buf, "%s/%s value is too long in control word at index %d; max length = %d, got %d",
      ...
      base64_encode(src, words[i].cw_len, sprintf_buf);
      l = (int)words[i].cw_len;
      break;
	...
	RETURN_ERROR;
      ...
      l = sprintf(sprintf_buf, "T%lu:%lu", (unsigned long)words[i].cw_timeval.tv_sec,
