 ...
 *
 * $Id: scamper_do_tracelb.c,v 1.216 2009/05/19 04:40:40 mjl Exp $
 *
...
#define SCAMPER_DO_TRACELB_DPORT_MIN       1
#define SCAMPER_DO_TRACELB_DPORT_DEF       (32768+666+1)
#define SCAMPER_DO_TRACELB_DPORT_MAX       65535
...
static const uint8_t MODE_FIRSTADDR  = 0; /* probing for the first address */
static const uint8_t MODE_FIRSTHOP   = 1; /* probing for the first hop */
static const uint8_t MODE_HOPPROBE   = 2; /* probing a hop for lb paths */
static const uint8_t MODE_PERPACKET  = 3; /* determine if router lb per pkt */
static const uint8_t MODE_BRINGFWD   = 4; /* need more flowids for hop-probe */
static const uint8_t MODE_BRINGFWD0  = 5; /* when first addr branches */
static const uint8_t MODE_CLUMP      = 6; /* clump of nodes */

#ifndef _WIN32
static const uint8_t MODE_RTSOCK     = 7; /* need to determine outgoing if */
#endif
...
typedef struct tracelb_newnode_t
{
...
#ifndef _WIN32
  scamper_fd_t            *rt;           /* route socket */
#endif

  scamper_dl_hdr_t        *dl_hdr;       /* datalink header to prepend on tx */
...
#define TRACE_OPT_WAITTIMEOUT  10
#define TRACE_OPT_WAITPROBE    11
...
const char *scamper_do_tracelb_usage(void)
{
  return "tracelb [-c confidence] [-d dport] [-f firsthop] [-g gaplimit]\n"
         ...
         "        [-t tos] [-w wait-timeout] [-W wait-probe]";
}
...
#define TRACELB_CONFIDENCE_MAX_N 20
...
{
  scamper_tracelb_t *trace = task->data;
  trace->error = err;
  ...
  for(i=0; i<set->probec; i++)
      ...
      for(j=0; j<probe->rxc; j++)
	{
	  ...
	  if(SCAMPER_TRACELB_REPLY_IS_ICMP_TTL_EXP(reply) == 0)
	    {
	      scamper_debug(__func__, "reply %d not time exceeded", i);
	      return 0;
	    }
	}
    }
...
#ifndef _WIN32
  assert(br->probec > 0 || br->mode == MODE_RTSOCK);
#else
  assert(br->probec > 0);
#endif
...
static void tracelb_path_distance_1(tracelb_path_t *path, int distance)
{
  ...
  scamper_debug(__func__, "path %d -> %d", path->distance, distance);
...
{
  scamper_tracelb_t *trace = task->data;
  tracelb_state_t *state = task->state;
  ...
  scamper_queue_wait_tv(task->queue, &next_tx);
  return;
}
...
static int tracelb_process_hops(scamper_task_t *task, tracelb_branch_t *br)
{
   ...
   * happened prior to teaching here.
   */
...
static void tracelb_process_probes(scamper_task_t *task, tracelb_branch_t *br)
{
  ...
  for(i=0; i<br->probec; i++)
    {
      ...
      scamper_debug(__func__, "flowid %d ttl %d rxc %d mode %s",
  ...
  if(br->mode == MODE_PERPACKET)
      ...
      for(i=0; i<=br->probec; i++)
	{
	   ...
	   */
	  if(pr->mode == MODE_HOPPROBE ||
	     pr->mode == MODE_FIRSTHOP ||
	     pr->mode == MODE_CLUMP)
	    {
	      hopprobe = 1;
				 ...
				 tracelb_probe_t *pr, scamper_addr_t *from)
{
 ...
 err:
  tracelb_branch_free(state, branch);
  if(node != NULL) scamper_tracelb_node_free(node);
				   ...
				   scamper_icmp_resp_t *ir)
{
  ...
  else if(trace->type == SCAMPER_TRACELB_TYPE_TCP_SPORT)
    {
      /* if the ICMP type is not something that we care for, then drop it */
      if(SCAMPER_ICMP_RESP_IS_TTL_EXP(ir) == 0 &&
	 SCAMPER_ICMP_RESP_IS_UNREACH(ir) == 0 &&
...
/*
 * handletimeout_rtsock
 *
 * if no route is received in response to a route socket query, then abandon
 */
#ifndef _WIN32
static void handletimeout_rtsock(scamper_task_t *task, tracelb_branch_t *br)
{
  tracelb_state_t *state = task->state;
  heap_delete(state->active, br->heapnode);
  tracelb_branch_free(state, br);
...
#ifndef _WIN32
  if(br->mode == MODE_RTSOCK)
    {
      handletimeout_rtsock(task, br);
      return;
    }
#endif
...
{
  //handle_perpacket(task, probe, from);
  return;
}
...
static void do_tracelb_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl)
{
   ...
   * an outgoing one.
   */
  if((dl->dl_tcp_flags & TH_SYN) != 0 && (dl->dl_tcp_flags & ~TH_SYN) == 0)
    {
      return;
    }
...
static int tracelb_handle_rt(scamper_task_t *task, tracelb_branch_t *br,
			     scamper_rt_rec_t *rt)
{
  scamper_tracelb_t *trace = task->data;
  tracelb_state_t *state = task->state;
   ...
   */
  if((state->dl_hdr = scamper_dl_hdr_alloc(state->dl, trace->src, trace->dst,
					   rt->gwaddr)) == NULL)
    {
      return -1;
    }
...
/*
 * do_tracelb_handle_rt
 *
 * a route lookup has been done for a destination IP address.  the result
 * has come back.  open the datalink socket matching the outbound interface.
 */
#ifndef _WIN32
static void do_tracelb_handle_rt(scamper_task_t *task, scamper_rt_rec_t *rt)
{
  tracelb_state_t  *state = task->state;
  tracelb_branch_t *br;
  ...
  /* there should be a single branch object */
  assert(heap_count(state->active) == 1);
  br = heap_remove(state->active);
  assert(br->mode == MODE_RTSOCK);

  /* don't need the routing socket any more */
  ...
  /* handle the response, and put the branch back in for probing */
  if(tracelb_handle_rt(task, br, rt) != 0 ||
     tracelb_branch_waiting(state, br) != 0)
    {
      tracelb_handleerror(task, errno);
      return;
    }
...
static void do_tracelb_write(scamper_task_t *task)
{
  ...
  if((outfile = scamper_outfiles_get(outfile_name)) != NULL)
    {
      ...
      scamper_file_write_tracelb(sf, (scamper_tracelb_t *)task->data);
    }
  ...
  if(trace->type == SCAMPER_TRACELB_TYPE_TCP_SPORT ||
      ...
      if((state->rt = scamper_fd_rtsock()) == NULL)
	{
      ...
      if(scamper_rtsock_getroute(trace->dst, &rr) != 0 ||
         tracelb_handle_rt(task, branch, &rr) != 0)
        {
          goto err;
        }
#endif
  ...
  if(trace->dst->type == SCAMPER_ADDR_TYPE_IPV4)
    {
      if(trace->type == SCAMPER_TRACELB_TYPE_UDP_DPORT)
	{
	  if((state->probe = scamper_fd_udp4(NULL, trace->sport)) == NULL)
	    goto err;
	}
      else if(trace->type == SCAMPER_TRACELB_TYPE_ICMP_ECHO)
	{
	  if((state->probe = scamper_fd_icmp4(NULL)) == NULL)
	    goto err;
	}
  ...
  else if(trace->dst->type == SCAMPER_ADDR_TYPE_IPV6)
    {
      if(trace->type == SCAMPER_TRACELB_TYPE_UDP_DPORT)
	{
	  if((state->probe = scamper_fd_udp6(NULL, trace->sport)) == NULL)
	    goto err;
	}
      else if(trace->type == SCAMPER_TRACELB_TYPE_ICMP_ECHO)
	{
	  if((state->probe = scamper_fd_icmp6(NULL)) == NULL)
	    goto err;
	}
      else if(trace->type == SCAMPER_TRACELB_TYPE_UDP_SPORT)
	{
...
static void do_tracelb_probe(scamper_task_t *task)
{
  ...
  if(branch->mode == MODE_RTSOCK)
    {
      if(scamper_rtsock_getroute(state->rt, trace->dst) == 0)
	{
	  /*
  ...
  if(trace->type == SCAMPER_TRACELB_TYPE_UDP_DPORT ||
  ...
  else if(trace->type == SCAMPER_TRACELB_TYPE_ICMP_ECHO)
    {
      ...
      probe.pr_icmp_sum  = tp->probe->flowid;

      /* fudge the checksum field so it is used as the flow id */
      memcpy(probe.pr_data, &tp->probe->flowid, 2);
    ...
    {
      assert(trace->type == SCAMPER_TRACELB_TYPE_TCP_SPORT);
...
static int tracelb_arg_param_validate(int optid, char *param, long *out)
{
  ...
  switch(optid)
    ...
    case TRACE_OPT_DPORT:
	 ...
	 tmp < SCAMPER_DO_TRACELB_DPORT_MIN ||
	 tmp > SCAMPER_DO_TRACELB_DPORT_MAX)
	{
	  goto err;
	}
      break;
...
int scamper_do_tracelb_init(void)
{
  ...
  funcs.task_addrs     = scamper_do_tracelb_dstaddr;

#ifndef _WIN32
  funcs.handle_rt      = do_tracelb_handle_rt;
#endif
